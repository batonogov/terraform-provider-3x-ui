...
- `sub_json_observatory` (Optional, String) - Observatory block injected into the JSON subscription for client-side balancers, as a JSON string. Added in 3x-ui v3.7.0; older panels report an empty string (unsupported). Like `sub_json_mux`, `sub_json_rules` and `sub_json_final_mask`, this value is read once when the subscription server starts, so a change applies to state but does not affect served subscriptions until the panel restarts ([#443](https://github.com/batonogov/terraform-provider-threexui/issues/443)).
