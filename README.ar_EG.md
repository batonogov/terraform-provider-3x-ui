...
- **النقل بين السيرفرات** — أعد إنشاء نفس الإعداد على VPS جديد بأمر `terraform apply` واحد.
- **Snapshot للوحة** — `terraform state pull` بيدّيك export كامل للـ inbounds و العملاء و الإعدادات.
...
| الانتقال لسيرفر جديد | إعادة إدخال يدوي | `terraform apply` على الـ endpoint الجديد |
...
**سياسة الدعم:** الـ provider بيدعم رسميًا أربعة سطور فرعية (minor) من 3x-ui: **2.9.x**، **3.0.x**، **3.1.x** و **3.2.x** — كل patch متنزّل من السطور دي بيتشغّل في matrix الـ acceptance في كل push على `main` و في كل pull request.
...
- [Backup-as-code](docs/guides/backup-as-code.md) — احتفظ بحالة اللوحة كاملة في git و استعد منها في ثوان.
- [نقل 3x-ui بين السيرفرات](docs/guides/server-migration.md) — انقل اللوحة لـ VPS جديد من غير إعادة إدخال أي حاجة.
