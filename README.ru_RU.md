...
**Политика поддержки:** провайдер официально поддерживает четыре минорные ветки 3x-ui: **2.9.x**, **3.0.x**, **3.1.x** и **3.2.x** — каждый выпущенный патч всех четырёх веток гоняется в acceptance-матрице на каждом push в `main` и на каждом pull request.
