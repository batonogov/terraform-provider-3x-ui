...
- **مهاجرت بین سرورها** — همان ست‌آپ را با یک `terraform apply` روی VPS جدید بازسازی کنید.
- **اسنپ‌شات از پنل** — `terraform state pull` خروجی کامل اینباندها، کلاینت‌ها و تنظیمات است.
...
| مهاجرت به سرور جدید | ورود دستی همه‌چیز | `terraform apply` به endpoint جدید |
...
**سیاست پشتیبانی:** این provider به‌طور رسمی از دو خط مینور آخر 3x-ui پشتیبانی می‌کند. در حال حاضر این یعنی **2.8.x** و **2.9.x** — هر patch منتشرشده از هر دو خط در هر push به `main` و هر pull request توسط ماتریس acceptance اجرا می‌شود. وقتی مینور جدیدی (مثلاً 2.10.0) منتشر شود، قدیمی‌ترین خط پشتیبانی‌شده از ماتریس و این جدول حذف می‌شود.
...
- [Backup-as-code](docs/guides/backup-as-code.md) — کل وضعیت پنل را در git نگه دارید و در چند ثانیه بازیابی کنید.
- [مهاجرت 3x-ui بین سرورها](docs/guides/server-migration.md) — کل پنل را بدون تایپ مجدد به VPS جدید منتقل کنید.
