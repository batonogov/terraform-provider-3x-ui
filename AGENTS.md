...
| REALITY `maxTimediff` key casing and units | The panel persists the field as `maxTimediff` while xray-core declares `json:"maxTimeDiff"` (`infra/conf/transport_security.go`); `encoding/json` matches case-insensitively, so the panel spelling still binds. It is a `uint64` in milliseconds (`transport/internet/reality/config.go` multiplies by `time.Millisecond`), hence `int64validator.AtLeast(0)` on `reality_settings.max_timediff`; `0` disables the time check |
