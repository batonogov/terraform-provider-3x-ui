...
| `xray_routing` rule list needs `ModifyPlan` reconciliation | `rule` is a `ListNestedBlock` whose nested attrs are `Optional+Computed` + `UseStateForUnknown`. Elements are matched **by index**, so on reorder/removal the modifier copies the prior rule's unset fields into the new rule at the same index — bleeding stale matchers across rules (e.g. a removed `geoip:private` ip rule's `ip` lands on the `domain` rule that takes its slot). `XrayRoutingResource.ModifyPlan` (`reconcileRoutingPlan`) overrides the planned `rule` list with the configured one so config is authoritative; safe because the panel echoes routing verbatim and read filters the managed `api`/`xui-dns-allow` rules, so there are no legitimate computed-only routing fields. `ModifyPlan` reads `rule` as `types.List` to tolerate an unknown collection (computed `dynamic "rule"`), deferring to schema modifiers when unknown. Regression-tested by `TestAccXrayRoutingRulesReorderNoFieldBleed`. The same `Optional+Computed+UseStateForUnknown` pattern exists on other `ListNestedBlock`s (`xray_outbounds`, `xray_dns`, `xray_basics`); only `xray_routing` reconciles today since its rules are routinely reordered/removed |
