...
1. **Клиент без email** — создаётся в API, но вызывает SQL-ошибку (`converting NULL to string`) при последующем addClient к тому же inbound. **Исправлено**: `email` сделан Required в schema (`resource_inbound_client.go`).
2. **Гонка при create+delete** — при одновременном создании новых клиентов и удалении старого, удалённый клиент может остаться в API. Mutex защищает от параллельных write-операций, но race condition возможна при пересечении create и delete. Это ограничение API 3x-ui, не провайдера.
